...
export const FlashcardsApp: React.FC<FlashcardsAppProps> = ({ userToken }) => {
    const [status, setStatus] = useState<FlashcardStatus>('GENERATE');
    ...
    const handleApprove = async (finalDeck: FlashDeck) => {
        ...
        try {
            const response = await fetch('/api/flashcards/save-deck', {
                method: 'POST',
                headers: {
            ...
            const data = await response.json();
            if (!response.ok) throw new Error(data.error || 'Erro ao salvar deck.');
        ...
        <div className="flash-app-host">
            ...
            {status === 'LOADING' && (
                <div className="flash-loading-state">
                    ...
                    <h3>Sintonizando com o Gemini...</h3>
                    <p>O Arquiteto de Software está extraindo o vocabulário estratégico para o seu aluno.</p>
                </div>
            )}
            ...
            {status === 'SAVED' && (
                <div className="flash-success-state">
                    ...
                    <div className="success-actions">
                        ...
                        <a href="/activities" className="btn-view">Ver Missões</a>
                    </div>
                </div>
            )}
