...
import { g as getItemById } from './store_BEgSzlvS.mjs';
...
const POST = async ({ request }) => {
  try {
    ...
    const { data: profile, error: profileError } = await supabaseAdmin.from("profiles").select("coins, inventory").eq("id", user.id).single();
