...
const GET = async ({ request }) => {
  ...
  const { data: activities, error: dbError } = await supabaseAdmin.from("activities").select(`
            id, 
            title, 
            type, 
            created_at, 
  ...
  const { data: students, error: stuError } = await supabaseAdmin.from("students").select("id, name, level, student_id").eq("teacher_id", user.id);
  if (stuError) {
